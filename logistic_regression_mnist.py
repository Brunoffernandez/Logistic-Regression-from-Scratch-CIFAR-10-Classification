...
#dimension of the problem2
p=x_train.shape[1]
#Compute the ranks of the matrices X and X^T
print("Rank of X:", np.linalg.matrix_rank(x_train))
print("Rank of X^T:", np.linalg.matrix_rank(x_train.T))
...
def logistic_regression_NR_penalized(features, target, num_steps, tolerance, lambda_0):
    ...
    for step in range(num_steps):  
        ...
        p = logistic(signal)
        # modified gradient
        gradient = - features.T @ (target - p) + 2 * lambda_0 * beta 
        if np.linalg.norm(gradient) < tolerance:
            return beta, step
            
        W = np.diag(p * (1 - p)) 
        beta += np.linalg.inv(features.T @ W @ features + 2 * lambda_0 * np.diag(np.ones_like(beta))) @ (features.T @ (target - p) - 2 * lambda_0 * beta)
    return beta, -1

tol = 1e-12
beta, step = logistic_regression_NR_penalized(x_train, y_train, 100, tol, lambda_0)

y_predicted = logistic_forecast(x_test, beta)

print(f'Accuracy: {prediction_accuracy(y_predicted, y_test)}')
print(f'Steps to reach gradient norm tolerance threshold {tol}: {step}')